...
/* ────── MAIN COMPONENT ────── */
const PHVNonMovingBOSReport: React.FC = () => {
    const { user } = useUser();
    ...
    /* ────── Fetch report data ──────
       Matches PHVNonMovingBOSController.GetReport(repYear, whCode) exactly --
    ...
    const handleViewReport = async () => {
        ...
        try {
            ...
            const url = `/misapi/api/phvnonmovingbos/report/${repYearParam}/${whCodeParam}`;
            ...
            const items: NonMovingBOSItem[] = json.data || [];
    ...
    const downloadCSV = () => {
        ...
        const titleRows = [
            `Cost center: ${costCtrIdDisplay}/${costCtrNameDisplay}    warehouse: ${selectedWarehouse}`,
        ...
        const headers = [
            ...
            "Minimum Selling Price of Total Quantity (LKR)",
        ];
        ...
        a.href = url;
        a.download = `PHV_NonMoving_BOS_${selectedDept?.DeptId}_${selectedWarehouse}_${selectedYear}.csv`;
        a.click();
        URL.revokeObjectURL(url);
    };

    /* ────── PDF print — paginated 14 rows/page. Every page ends with the
       Date/Store keeper + Certified Correct + Engineer/Unit head signature
       block. The full Board of Survey recommendation + member signatures +
       DGM/AGM approval lines appear only on the final page. ────── */
    const printPDF = () => {
        if (reportData.length === 0) return;

        const totalPages = Math.ceil(sortedData.length / PRINT_ROWS_PER_PAGE) || 1;

        const buildRows = (pageIndex: number) => {
            const start = pageIndex * PRINT_ROWS_PER_PAGE;
            const pageItems = sortedData.slice(start, start + PRINT_ROWS_PER_PAGE);
    ...
    <div class="title">Board of survey Recommendation for non moving stocks - Annual verification of the year ${selectedYear}</div>`;
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width:12%;">Sheet No</th>
              <th style="width:10%;">Batch Id</th>
              <th style="width:9%;">Quantity</th>
              <th style="width:10%;">Unit Price</th>
              <th style="width:13%; font-size:7.5px;">Recommendation of BOS<br/>S-to be sold &nbsp; D-to be destroyed</th>
              <th style="width:12%;">Minimum selling price<br/>of total quantity (LKR)</th>
            </tr>
          </thead>
          <tbody>${buildRows(p)}</tbody>
        </table>
        ${buildPageFooter()}
        ${isLastPage ? buildBOSFooter() : ""}
      </div>`;
        }
...
<html>
<head>
  <title>Board of survey Recommendation for non moving stocks - ${selectedYear}</title>
  <style>
    @media print {
      @page { margin: 10mm 8mm 14mm 8mm; }
      body { margin:0; font-family:Arial,Helvetica,sans-serif; font-size:9.5px; color:#111; }
      .page-break { page-break-after: always; }
      .header-row { display:flex; justify-content:space-between; font-size:9.5px; margin-bottom:6px; }
      .title { text-align:center; font-weight:bold; font-size:10.5px; margin:6px 0 10px; }
      table { border-collapse:collapse; width:100%; font-size:8px; margin-bottom:6px; }
      ...
      .sig-row { display:flex; justify-content:space-between; margin-top:14px; font-size:8.5px; text-align:center; page-break-inside:avoid; }
      .sig-col { width:45%; }
      .sig-space { height:26px; }
      hr { border:none; border-top:1px solid #000; margin:10px 0; }
      .certified-title { text-align:center; font-weight:bold; font-size:10px; page-break-inside:avoid; }
      .board-title { text-align:center; font-weight:bold; font-size:10.5px; margin:16px 0 6px; page-break-inside:avoid; }
      .bos-note { font-size:8.5px; text-align:justify; margin-bottom:10px; }
      .signed-by { font-size:8.5px; margin-bottom:6px; }
      .member-block { margin-bottom:8px; page-break-inside:avoid; }
      .member-row { display:flex; align-items:baseline; font-size:8.5px; }
      .member-num { width:22%; }
      ...
      .approval-row { display:flex; justify-content:space-between; font-size:8.5px; padding:4px 0; }
    }
  </style>
</head>
<body>
        ...
        <div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
            <div className="flex justify-between items-center mb-4">
                <h2 className={`text-xl font-bold ${maroon}`}>
                    7. Physical Verification Non-Moving WH Wise (FIFO) - AV/6/BOS
                </h2>
            </div>
                    ...
                    <div className="relative bg-white w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] xl:w-[75vw] max-w-7xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-32 lg:mt-40 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
                        {reportLoading && (
                            <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-sm text-gray-600">Fetching non-moving stock records from server</p>
                            </div>
                        )}
                        {!reportLoading && reportData.length > 0 && (
                            <div className="p-2 md:p-2 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible print:mt-10 print:ml-12">
                                ...
                                <div className="flex justify-between text-sm ml-5 mr-12 mb-2">
                                    <div>
                                        <span className="font-bold">Cost center:</span>
                                        {costCtrIdDisplay}/{costCtrNameDisplay}
                                    </div>
                                    <div>
                                ...
                                <h2 className={`text-base md:text-lg font-bold text-center my-4 ${maroon}`}>
                                    Board of survey Recommendation for non moving stocks - Annual verification of the year {selectedYear}
                                </h2>
                                    ...
                                    <div className="min-w-[1300px]">
                                        <table className="w-full text-xs border-collapse">
                                            <thead className={`${maroonGrad} text-white`}>
                                                <tr>
                                                    <th className="px-3 py-2 border border-gray-300" style={{ width: "5%" }}>
                                                        Serial No
                                                    </th>
                                                    <th className="px-3 py-2 border border-gray-300" style={{ width: "10%" }}>
                                                        Code No
                                                    </th>
                                                    <th className="px-3 py-2 border border-gray-300" style={{ width: "19%" }}>
                                                        Description
                                                    </th>
                                ...
                                {/* On-screen preview of the print footer (shown once; the printed
                    version repeats the signature block on every 14-row page) */}
                                <div className="ml-5 mr-12 mt-6 text-xs md:text-sm">
                                    ...
                                    <p className={`text-center font-bold ${maroon}`}>Certified Correct</p>
                                    ...
                                    <p className={`text-center font-bold mt-6 mb-2 ${maroon}`}>Recommendation of Board Survey</p>
                                    ...
                                    <div className="flex justify-between mb-2">
                                        <span>Date &nbsp; : ..................................</span>
                                        <span>Cheif Opperative Officer &nbsp;&nbsp; ..............................</span>
                                    </div>
                                </div>
                            </div>
                        )}
...
export default PHVNonMovingBOSReport;