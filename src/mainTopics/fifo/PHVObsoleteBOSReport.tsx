...
/* ────── MAIN COMPONENT ────── */
const PHVObsoleteBOSReport: React.FC = () => {
  const { user } = useUser();
  ...
  /* ── Report state ── */
  const [reportData, setReportData] = useState<ObsoleteIdleItem[]>([]);
  ...
  /* ────── Fetch report data ──────
     Matches PHVObsoleteBOSController.GetReport(repYear, whCode) exactly --
  ...
  const handleViewReport = async () => {
    ...
    try {
      ...
      const url = `/misapi/api/phvobsoletebos/report/${repYearParam}/${whCodeParam}`;
      ...
      const items: ObsoleteIdleItem[] = json.data || [];
  ...
  const downloadCSV = () => {
    ...
    const headers = [
      ...
      "Minimum Selling Price of Total Quantity (Rs.)",
    ];
    ...
    a.href = url;
    a.download = `PHV_ObsoleteIdle_BOS_${selectedDept?.DeptId}_${selectedWarehouse}_${selectedYear}.csv`;
    a.click();
...
<html>
<head>
  <title>Board of Survey Recommendation for Obsolete and Idle Stocks Items - ${selectedYear}</title>
  <style>
    @media print {
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width:12%; font-size:7.5px;">Recommendation of BOS<br/>S-be Sold &nbsp; D-To be Destroyed</th>
        <th style="width:12%;">Minimum selling price<br/>of Total Quantity (Rs.)</th>
      </tr>
    </thead>
          ...
          <div className="relative bg-white w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] xl:w-[75vw] max-w-7xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-32 lg:mt-40 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
            {reportLoading && (
              <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-sm text-gray-600">Fetching obsolete/idle stock records from server</p>
              </div>
            )}
                ...
                <div className="ml-5 mt-1 mb-5 border border-gray-200 rounded-lg overflow-x-auto print:ml-12 print:mt-12 print:overflow-visible">
                  <div className="min-w-[1300px]">
                    <table className="w-full text-xs border-collapse">
                      <thead className={`${maroonGrad} text-white`}>
                        <tr>
                          ...
                          <th className="px-3 py-2 border border-gray-300" style={{ width: "12%" }}>
                            Recommendation of BOS (S-be Sold, D-To be Destroyed)
                          </th>
                          <th className="px-3 py-2 border border-gray-300" style={{ width: "12%" }}>
                            Minimum selling price of Total Quantity (Rs.)
                          </th>
                    ...
                    <p className="text-xs text-gray-500 mt-2 text-right px-2">
                      Total records: {reportData.length.toLocaleString()} &nbsp;|&nbsp; Reference total (Unit Price
                      &times; Quantity): Rs. {formatNumber(grandTotalStockBook)}
                    </p>
                  </div>
                </div>
...
export default PHVObsoleteBOSReport;