...
const PHVObsoleteIdleFIFO: React.FC = () => {
  ...
  const handleDownloadCSV = () => {
    if (reportData.length === 0 || !selectedDept) {
      toast.error("No report data available to download as CSV.");
      return;
    }

    const csvRows: string[] = [
      `"STATEMENT OF OBSOLETE AND IDLE MATERIALS IN STOCKS - ${selectedYear}"`,
      `"Cost Centre: ${selectedDept.DeptId} - ${selectedDept.DeptName} WARE HOUSE - ${selectedWarehouse}"`,
      "",
      [
        "SerialNo",
        "Code No",
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `PHV_Obsolete_Idle_FIFO_${selectedDept.DeptId}_${selectedWarehouse}_${selectedYear}_${selectedMonth?.toString().padStart(2, "0") || "00"}.csv`;
    link.click();
