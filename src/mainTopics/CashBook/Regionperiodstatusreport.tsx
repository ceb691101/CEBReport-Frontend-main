...
interface PeriodStatusItem {
	...
	CompNm: string | null; // Region/Division name
}
...
/* ────── MAIN COMPONENT ────── */
const RegionPeriodStatusReport: React.FC = () => {
	const {user} = useUser();
	const epfNo = user?.Userno || "";

	/* ── Region list state ── */
	const [regions, setCompanies] = useState<Company[]>([]);
	const [filteredCompanies, setFilteredCompanies] = useState<Company[]>([]);
	const [searchId, setSearchId] = useState("");
	const [searchName, setSearchName] = useState("");
	const [page, setPage] = useState(1);
	const [regionLoading, setLoading] = useState(true);
	const [regionError, setError] = useState<string | null>(null);
	...
	/* ── Report state ── */
	const [selectedRegion, setSelectedRegion] = useState<Company | null>(null);
	...
	/* ────── Fetch report ────── */
	const fetchReport = async (region: Company) => {
		...
		try {
			const regionParam = encodeURIComponent(region.compId);
			const url = `/misapi/api/regionperiodstatus/report/${selectedYear}/${selectedMonth}/${regionParam}`;
	...
	const regionName = reportData.find((r) => r.CompNm)?.CompNm || selectedRegion?.CompName || "";
	...
	const downloadCSV = () => {
		...
		a.href = url;
		a.download = `RegionPeriodStatus_${selectedYear}_${selectedMonth}_${regionDisplay}.csv`;
		a.click();
...
<body>
  ...
  <div class="info">
    <strong>Division/Region:</strong> ${regionDisplay}/${regionName}
  </div>
  <table>
    <thead>
      <tr>
	...
	return (
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>Region Wise Period Status</h2>
			</div>
			...
			{regionLoading && (
				<div className="flex flex-col items-center justify-center py-12">
					...
					<p className="mt-3 text-gray-600 text-sm">Loading divisions/regions...</p>
				</div>
			)}
			...
			{!regionLoading && !regionError && filteredCompanies.length > 0 && (
				<>
					<div className="overflow-x-auto rounded-lg border border-gray-200">
						<div className="max-h-[50vh] overflow-y-auto">
                            ...
                            ${selectedRegion?.compId === region.compId ? "bg-green-600 text-white" : `${maroonGrad} text-white`}`}
												>
						...
						{!reportLoading && reportData.length > 0 && (
							<div className="p-2 md:p-2 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible print:mt-10 print:ml-12">
								...
								<div className="text-sm mb-3 ml-5 mr-12">
									<span className="font-bold">Division/Region:</span> {regionDisplay}/{regionName}
								</div>
...
export default RegionPeriodStatusReport;