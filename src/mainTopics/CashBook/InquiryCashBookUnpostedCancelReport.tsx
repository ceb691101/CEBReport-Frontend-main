// InquiryCashBookUnpostedCancelReport.tsx
...
interface InquiryCashBookUnpostedCancelItem {
	DocNo: string | null;
	DocDt: string | null;
...
/* ────── MAIN COMPONENT ────── */
const InquiryCashBookUnpostedCancelReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryCashBookUnpostedCancelItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquirycashbookunpostedcancel/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryCashBookUnpostedCancelItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY doc_no,ent_by ────── */
	const sortedData = [...reportData].sort(
		(a, b) =>
			(a.DocNo || "").localeCompare(b.DocNo || "") ||
			(a.EntBy || "").localeCompare(b.EntBy || "")
	);
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Cost Center wise Un-posted cancellation (Pay Slips) Inquiry Report From ${fromDate} To ${toDate}`,
		...
		sortedData.forEach((it, i) => {
			rows.push(
				[
					...
					csvEscape(it.RejBy),
					csvEscape(formatDate(it.RejcDt)),
					csvEscape(it.CancelledUser),
					csvEscape(formatDate(it.CancelDt)),
				].join(",")
			);
		});
		...
		a.href = url;
		a.download = `InquiryCashBookUnpostedCancel_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Cost Center wise Un-posted cancellation (Pay Slips) Inquiry Report From ${fromDate} To ${toDate}</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:10%;">Payslips No.</th>
        <th style="padding:5px 6px; width:9%;">Transaction Date</th>
        <th style="padding:5px 6px; width:9%; text-align:right;">Amount</th>
        <th style="padding:5px 6px; width:10%;">Entered By</th>
        <th style="padding:5px 6px; width:12%;">Pay Slip Status</th>
        <th style="padding:5px 6px; width:10%;">Rejected By</th>
        <th style="padding:5px 6px; width:9%;">Rejected Date</th>
        <th style="padding:5px 6px; width:12%;">Cancelled User</th>
        <th style="padding:5px 6px; width:9%;">Cancelled Date</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
      <tr>
        ...
        <td colspan="6" style="border:1px solid #d1d5db;"></td>
      </tr>
    </tfoot>
  </table>
		...
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>
					Inquiry - Cash Book (Up post Cancellation)
				</h2>
			</div>
											...
											<tbody>
												{sortedData.map((it, i) => (
													...
													>
														...
														<td className="px-4 py-2 text-center border-r border-gray-300">
															{formatDate(it.RejcDt)}
														</td>
														<td className="px-4 py-2 border-r border-gray-300">
															{it.CancelledUser || ""}
														</td>
														<td className="px-4 py-2 text-center border-r border-gray-300">
															{formatDate(it.CancelDt)}
														</td>
													</tr>
												))}
											</tbody>
...
export default InquiryCashBookUnpostedCancelReport;