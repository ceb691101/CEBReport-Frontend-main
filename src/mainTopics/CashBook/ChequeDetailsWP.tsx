...
/* ────── MAIN COMPONENT ────── */
const ChequeDetailWPReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<ChequeDetailWPItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/chequedetailwp/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: ChequeDetailWPItem[] = json.data || [];
	...
	const downloadCSV = () => {
		...
		rows.push(
			`Grand Total,,,,,${csvEscape(formatNumber(grandTotalDrAmt))},${csvEscape(
		...
		a.href = url;
		a.download = `ChequeDetailsWP_${fromDate}_${toDate}.csv`;
		a.click();
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8.5px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:6px 8px; width:8%;">Cheque Date</th>
        <th style="padding:6px 8px; width:9%;">Cheque No</th>
        <th style="padding:6px 8px; width:11%;">Payslip No</th>
        <th style="padding:6px 8px; width:8%;">Acct. Code</th>
        <th style="padding:6px 8px; width:10%; text-align:right;">Amount</th>
        <th style="padding:6px 8px; width:10%; text-align:right;">Total Chq</th>
        <th style="padding:6px 8px; width:15%;">Payee</th>
        <th style="padding:6px 8px; width:15%;">Remarks</th>
        <th style="padding:6px 8px; width:10%;">Address</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
      <tr>
        ...
        <td colspan="3" style="padding:6px 8px; border:1px solid #d1d5db;"></td>
      </tr>
    </tfoot>
  </table>
...
export default ChequeDetailWPReport;