...
import { PieChart as ReChartsPie, Cell, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, BarChart, Bar } from "recharts";
...
const COLORS = ["#7A0000", "#620000", "#A5522D", "#D4A574"];
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="rounded-full bg-[#7A0000]/10 p-3">
              <Shield className="h-6 w-6 text-[#7A0000]" />
            </div>
            <div>
              <h1 className="text-4xl font-bold tracking-tight text-stone-900">Admin Dashboard</h1>
              <p className="mt-1 text-sm text-stone-600">Real-time system overview and management</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, idx) => (
            <div
              key={stat.label}
              className={`group rounded-[24px] border border-stone-200 ${stat.bgColor} p-6 shadow-[0_8px_24px_rgba(70,40,20,0.04)] transition-all duration-300 hover:shadow-[0_16px_40px_rgba(70,40,20,0.12)] hover:-translate-y-1 overflow-hidden relative`}
        ...
        {/* Topic Section */}
        <section className="mb-8 pb-6 border-b-2 border-stone-200/50">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-1">
              <h3 className="text-2xl font-bold text-stone-900">⚡ Quick Access</h3>
              <p className="text-sm text-stone-500">One-tap access to key admin modules</p>
            </div>
          </div>
        </section>
        ...
        {!isLoading && (
          <section className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-[28px] border border-stone-200 bg-gradient-to-br from-white to-stone-50/50 p-8 shadow-[0_16px_50px_rgba(70,40,20,0.06)] hover:shadow-[0_24px_60px_rgba(70,40,20,0.1)] transition-all duration-300">
                ...
                <div className="relative">
                  <ResponsiveContainer width="100%" height={320}>
                    <ReChartsPie data={categoryData} cx="50%" cy="50%" outerRadius={110} innerRadius={60}>
            ...
            {/* Users panel (left) and Quick Access (right) */}
              ...
              <>
                <div className="rounded-[28px] border border-stone-200 bg-white p-8 shadow-[0_12px_40px_rgba(70,40,20,0.04)]">
                  <div className="h-[440px] flex flex-col justify-between">
                    <div>
                      <ResponsiveContainer width="100%" height={320}>
                        <ReChartsPie data={userBreakdown} cx="50%" cy="50%" outerRadius={120} innerRadius={60}>
                    ...
                    <div className="mt-2 flex items-center justify-center gap-8">
                      {userBreakdown.map((u: any, i: number) => (
                        <div key={u.name} className="flex items-center gap-2.5">
                          ...
                          <span className="text-sm font-medium text-stone-700">{u.name}: {u.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="rounded-[28px] border border-stone-200 bg-white p-6 shadow-[0_12px_40px_rgba(70,40,20,0.04)]">
                  <div className="grid gap-4 sm:grid-cols-2">
                    {quickLinks.map((item) => (
                      <button
                        key={`quick-inline-${item.title}`}
                        type="button"
                        onClick={() => navigate(item.path)}
                        className="group rounded-[20px] border border-stone-100 bg-gradient-to-br from-white to-stone-50/50 p-6 text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(0,0,0,0.08)] hover:border-stone-200"
                      >
                        <div className="flex items-start justify-between">
                          <div className={`rounded-[16px] ${item.bgColor} p-3 transition-transform group-hover:scale-110`}>
                            <item.icon className={`h-6 w-6 ${item.color}`} />
                          </div>
                          <ArrowRight className={`h-4 w-4 ${item.color} opacity-30 transition-all group-hover:opacity-100 group-hover:translate-x-1`} />
                        </div>
                        <h3 className="mt-4 text-lg font-semibold text-stone-900">{item.title}</h3>
                        <p className="mt-2 text-sm text-stone-600 leading-relaxed line-clamp-2">{item.description}</p>
                      </button>
                    ))}
                  </div>
                </div>
