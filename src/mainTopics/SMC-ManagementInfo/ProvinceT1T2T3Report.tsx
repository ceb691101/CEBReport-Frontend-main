...
interface ProvinceT1T2T3Summary {
    fromDate: string;
    toDate: string;
    compId: string;
    totalRecords: number;
...
const getT2 = (it: ProvinceT1T2T3Item): number | null =>
...
const ProvinceT1T2T3ReportTable: React.FC<{
    data: ProvinceT1T2T3Item[];
    summary: ProvinceT1T2T3Summary | null;
    fromDate: string;
    toDate: string;
    compId: string;
    onClose: () => void;
    ...
    const provinceLabel = data[0]?.CompName || compId;
...
<body>
  ...
  <div class="title">Province Wise T1, T2, T3 Report From ${fromLabel} To ${toLabel}</div>
  <div class="info"><strong>Province:</strong> ${compId} / ${provinceLabel} &nbsp;&nbsp; <strong>Records:</strong> ${totalRecords}</div>
  <table>
    <thead>
      <tr>
        <th>Item</th>
  ...
  <div class="sign-block">
    <div class="sign-row">
      ...
      <div class="sign-col"><div class="sign-line">Certified By (Province Engineer)</div></div>
            ...
            <div className="relative bg-white w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] xl:w-[75vw] max-w-7xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-32 lg:mt-40 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
                <div className="p-4 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible print:mt-10 print:ml-12">
                    ...
                    <h3 className={`text-sm md:text-base font-semibold text-center mb-4 ${maroon}`}>
                        Province Wise T1, T2, T3 Report From {fromLabel} To {toLabel}
                    </h3>

                    <div className="flex flex-col sm:flex-row justify-between text-sm mb-4 gap-2 px-2">
                        <div>
                            <span className="font-bold">Province:</span> {compId} / {provinceLabel}
                        </div>
                    ...
                    <div className="mt-8 mb-2 px-2 print:hidden">
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-6 text-xs">
                            ...
                            <div>
                                <div className="border-b border-gray-400 h-8"></div>
                                <div className="mt-1 text-gray-600 font-semibold">Certified By</div>
                            </div>
                            <div>
                                <div className="border-b border-gray-400 h-8"></div>
...
/* ────── MAIN COMPONENT ────── */
const ProvinceT1T2T3Report: React.FC = () => {
    const { user } = useUser();
    ...
    const [reportData, setReportData] = useState<ProvinceT1T2T3Item[]>([]);
    const [reportSummary, setReportSummary] = useState<ProvinceT1T2T3Summary | null>(null);
    const [showReport, setShowReport] = useState(false);
    const [reportLoading, setReportLoading] = useState(false);

    /* ────── Fetch report for the selected province/company ────── */
    const handleViewReport = async (company: { id: string; name: string }) => {
        ...
        try {
            const url = `/misapi/api/provincet1t2t3/report?fromDate=${fromDate}&toDate=${toDate}&compId=${encodeURIComponent(
                company.id
            )}`;
            ...
            const items: ProvinceT1T2T3Item[] = json.data || [];
            if (items.length === 0) {
    ...
    return (
        ...
        >
            <h2 className={`text-xl font-bold mb-4 ${maroon}`}>Province Wise T1, T2, T3 Report</h2>
            ...
            <div className="mt-6">
                <ReusableCompanyList
                    ...
                    idColumnTitle="Province Code"
                    nameColumnTitle="Province Name"
                    loadingMessage="Loading Provinces..."
                    emptyMessage="No Provinces available for selection."
                />
            </div>
...
export default ProvinceT1T2T3Report;