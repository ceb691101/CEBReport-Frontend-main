...
interface JobSummaryPeriodItem {
    PrjAssDt: string | null;
    ProjectNo: string | null;
    EstimateNo: string | null;
    StandardCost: number | null;
    EstimateCost: number | null;
    // Can be null when the project has no matching transaction rows at all (not the same
    // as an actual cost of zero) - see JobSummaryPeriodDAL/Model notes.
    Actual: number | null;
    Descr: string | null;
    CctName: string | null;
}

interface JobSummaryPeriodSummary {
    fromDate: string;
    toDate: string;
    costCtr: string;
    totalRecords: number;
...
// Formats an amount, or "-" when the value is null (distinguishing "no data" from a real
// zero, since Actual can be null for projects with no transaction rows yet).
const formatAmountOrDash = (val: number | null | undefined): string => {
...
const JobSummaryPeriodTable: React.FC<{
    data: JobSummaryPeriodItem[];
    summary: JobSummaryPeriodSummary | null;
    fromDate: string;
    toDate: string;
    costCenter: string;
    ...
    const reportTitle = `Cost center wise job summary from ${fromLabel} To ${toLabel}`;
    const costCenterLine = `Cost Center : ${costCenter}${departmentName ? " - " + departmentName : ""}`;

    const sortedData = [...data]; // already ordered by backend (ORDER BY project_no)
    const totalRecords = summary?.totalRecords ?? sortedData.length;

    const variation = (it: JobSummaryPeriodItem): number | null =>
        it.Actual == null ? null : (it.StandardCost ?? 0) - it.Actual;
    ...
    const downloadCSV = () => {
        ...
        const rows = sortedData.map((it, i) =>
            [
                ...
                csvEscape(formatAmountOrDash(it.Actual)),
                csvEscape(formatAmountOrDash(variation(it))),
                csvEscape(it.Descr),
            ].join(",")
        );
        ...
        a.href = url;
        a.download = `JobSummaryPeriod_${costCenter}_${fromDate}_${toDate}.csv`;
        a.click();
    ...
    const printPDF = () => {
        ...
        <tr class="${i % 2 ? "bg-white" : "bg-gray-50"}">
          ...
          <td style="text-align:right;">${formatAmountOrDash(it.Actual)}</td>
          <td style="text-align:right;">${formatAmountOrDash(variation(it))}</td>
          <td>${it.Descr || ""}</td>
        </tr>`;
        });
...
<head>
  <style>
    @media print {
      ...
      .signoff { margin-top:24px; display:flex; justify-content:space-between; padding:0 15px; font-size:9px; }
      @page {
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Job No</th>
        <th>Project Date</th>
        <th style="text-align:right;">Stand. Cost</th>
        <th style="text-align:right;">Actual Cost</th>
        <th style="text-align:right;">Variation (S/C - Actual)</th>
        <th>Description</th>
      </tr>
    </thead>
                            ...
                            <table className="w-full text-xs border-collapse">
                                <thead>
                                    <tr className="bg-[#7A0000] text-white sticky top-0">
                                        ...
                                        <th className="px-3 py-2 border border-gray-300 font-bold text-right">Stand. Cost</th>
                                        <th className="px-3 py-2 border border-gray-300 font-bold text-right">Actual Cost</th>
                                        <th className="px-3 py-2 border border-gray-300 font-bold text-right">Variation (S/C - Actual)</th>
                                ...
                                <tbody>
                                    {sortedData.map((it, i) => (
                                        <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                            ...
                                            <td className="px-3 py-2 border-r border-gray-300 text-right font-mono">{formatAmount(it.StandardCost)}</td>
                                            <td className="px-3 py-2 border-r border-gray-300 text-right font-mono">{formatAmountOrDash(it.Actual)}</td>
                                            <td className="px-3 py-2 border-r border-gray-300 text-right font-mono">{formatAmountOrDash(variation(it))}</td>
...
/* ────── MAIN COMPONENT ────── */
const JobSummaryPeriodReport: React.FC = () => {
    const { user } = useUser();
    ...
    const [reportData, setReportData] = useState<JobSummaryPeriodItem[]>([]);
    const [reportSummary, setReportSummary] = useState<JobSummaryPeriodSummary | null>(null);
    ...
    const fetchReport = async (dept: Department) => {
        ...
        try {
            const url = `/misapi/api/jobsummaryperiod/report?fromDate=${fromDate}&toDate=${toDate}&costCtr=${dept.DeptId}`;
            ...
            const items: JobSummaryPeriodItem[] = json.data || [];
            if (items.length === 0) {
    ...
    const paginated = filtered.slice((page - 1) * PAGE_SIZE_LOCAL, page * PAGE_SIZE_LOCAL);
                ...
                <>
                    ...
                    <div className="flex justify-end items-center gap-3 mt-3">
                        ...
                        <span className="text-xs text-gray-600">
                            Page {page} of {Math.ceil(filtered.length / PAGE_SIZE_LOCAL)}
                        </span>
                        <button
                            onClick={() =>
                                setPage((p) =>
                                    Math.min(Math.ceil(filtered.length / PAGE_SIZE_LOCAL), p + 1)
                                )
                            }
                            disabled={page >= Math.ceil(filtered.length / PAGE_SIZE_LOCAL)}
                            className="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-600 text-xs hover:bg-gray-100 disabled:opacity-40 transition"
                        >
                            Next
                        </button>
...
export default JobSummaryPeriodReport;