...
/* ────── MAIN COMPONENT ────── */
const BranchGrnNotGenReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<BranchGrnNotGenItem[]>([]);
	...
	// Update the URL below to your actual branch/company list API if different.
	useEffect(() => {
		const fetchBranches = async () => {
			if (!epfNo) {
				setBranchError("No EPF number available.");
			...
			try {
				...
				const raw = Array.isArray(parsed)
					? parsed
					: parsed.data || [];
				const list: Branch[] = raw.map((c: any) => ({
					CompId: c.CompId,
					CompName: c.CompName,
				}));
				setBranches(list);
	...
	/* ────── Filter Branches ────── */
	useEffect(() => {
		const f = branches.filter(
			(b) =>
				(!searchId ||
					b.CompId.toLowerCase().includes(searchId.toLowerCase())) &&
				(!searchName ||
					b.CompName.toLowerCase().includes(searchName.toLowerCase()))
		);
		setFiltered(f);
		setPage(1);
	}, [searchId, searchName, branches]);
	...
	/* ────── Fetch report for a selected Branch ────── */
	const fetchReport = async (branch: Branch) => {
		...
		try {
			const compIdParam = encodeURIComponent(branch.CompId);
			const url = `/misapi/api/branchgrnnotgen/report/${fromDate}/${toDate}/${compIdParam}`;
			...
			const items: BranchGrnNotGenItem[] = json.data || [];
	...
	const branchName =
		reportData.find((r) => r.BranchName)?.BranchName || selectedBranch?.CompName || "";
	const branchDisplay = selectedBranch?.CompId || "";
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Branch wise GRN not generated issue - from ${fromDate} to ${toDate}`,
			`Branch/Province : ${branchDisplay}/${branchName}`,
			"",
		];
		...
		a.href = url;
		a.download = `BranchGrnNotGen_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Branch wise GRN not generated issue - from ${fromDate} to ${toDate}</div>
  <div class="info">
    <div><strong>Branch/Province :</strong> ${branchDisplay}/${branchName}</div>
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8.5px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:6px 8px; width:16%;">Document No</th>
        <th style="padding:6px 8px; width:12%;">Transaction</th>
        <th style="padding:6px 8px; width:13%;">Designation Dept</th>
        <th style="padding:6px 8px; width:21%;">Reference 1</th>
        <th style="padding:6px 8px; width:14%;">Warehouse Code</th>
        <th style="padding:6px 8px; width:19%; text-align:right;">Total</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
		...
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>
					Branch Wise GRN Not Generated Issues
				</h2>
			</div>
			...
			{!branchLoading && !branchError && filtered.length > 0 && (
				<>
					<div className="overflow-x-auto rounded-lg border border-gray-200">
						<div className="max-h-[50vh] overflow-y-auto">
                            ...
                            ${
											selectedBranch?.CompId === branch.CompId &&
											reportLoading
										...
										<table className="w-full text-xs border-collapse">
											<thead className={`${maroonGrad} text-white`}>
												<tr>
													...
													<th className="px-4 py-2 border border-gray-300" style={{width: "21%"}}>
														Reference 1
													</th>
													<th className="px-4 py-2 border border-gray-300" style={{width: "14%"}}>
														Warehouse Code
													</th>
													<th className="px-4 py-2 border border-gray-300 text-right" style={{width: "19%"}}>
														Total
													</th>
												</tr>
											</thead>
...
export default BranchGrnNotGenReport;