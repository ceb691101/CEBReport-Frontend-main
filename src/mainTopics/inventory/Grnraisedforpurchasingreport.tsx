...
interface GrnRaisedForPurchasingItem {
  MatCd: string | null;
  MatNm: string | null;
  Qty: number | null;
  Value: number | null;
...
const GrnRaisedForPurchasingReport: React.FC = () => {
  ...
  const [reportData, setReportData] = useState<GrnRaisedForPurchasingItem[]>([]);
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const url = matCodeSnapshot
        ? `/misapi/api/grnraisedforpurchasing/report/${fromSnapshot}/${toSnapshot}/${encodeURIComponent(matCodeSnapshot)}`
        : `/misapi/api/grnraisedforpurchasing/report/${fromSnapshot}/${toSnapshot}`;
      ...
      const mapped: GrnRaisedForPurchasingItem[] = rows.map((item: any) => ({
      ...
      setPeriodSummary({
        totalGrnValue:
          summaryRaw.totalGrnValue !== undefined && summaryRaw.totalGrnValue !== null
            ? Number(summaryRaw.totalGrnValue)
            : mapped.reduce((s, r) => s + (r.Value || 0), 0),
        grnCount: summaryRaw.grnCount !== undefined && summaryRaw.grnCount !== null ? Number(summaryRaw.grnCount) : 0,
        issueCount: summaryRaw.issueCount !== undefined && summaryRaw.issueCount !== null ? Number(summaryRaw.issueCount) : 0,
        issueTotal: summaryRaw.issueTotal !== undefined && summaryRaw.issueTotal !== null ? Number(summaryRaw.issueTotal) : 0,
      });
  ...
  const totalGrnValue = periodSummary?.totalGrnValue ?? reportData.reduce((s, r) => s + (r.Value || 0), 0);
  ...
  const handleExportCsv = () => {
    ...
    const titleRows = [
      `All (GRN) Raised by EDL for Purchasing`,
      `From ${resolvedFromDate} To ${resolvedToDate}`,
      `Material Code Filter: ${resolvedMatCode || "(All Materials)"}`,
      "",
    ];

    const headers = ["No", "Material Code", "Material Name", "Quantity", "Total Value"];
    ...
    reportData.forEach((r, i) => {
      ...
      rows.push(
        [
          ...
          csvEscape(formatMoney(r.Value)),
        ].join(",")
      );
    });

    rows.push(
      `Grand Total,,,,${csvEscape(formatQty(totalQty))},${csvEscape(formatMoney(totalGrnValue))}`
    );

    rows.push(
      `Total GRN value for the period,${csvEscape(formatMoney(totalGrnValue))}`
    );
    rows.push(
      `** No. of GRNs raised for the period,${csvEscape(periodSummary?.grnCount ?? 0)}`
    );
    rows.push(
      `** No. of issues raised for jobs and maintenance for the period,${csvEscape(periodSummary?.issueCount ?? 0)}`
    );
    rows.push(
      `** Total issues value for the period,${csvEscape(formatMoney(periodSummary?.issueTotal ?? 0))}`
    );
    rows.push(``);
    rows.push(`** For all materials raised for the period`);
    ...
    a.href = url;
    a.download = `GrnRaisedForPurchasing_${resolvedFromDate}_${resolvedToDate}.csv`;
    a.click();
    ...
    reportData.forEach((r, i) => {
      rows += `
        <tr class="${i % 2 ? "bg-white" : "bg-gray-50"}">
          ...
          <td class="px-3 py-2 border-r border-gray-300 text-right text-xs font-mono">${formatMoney(r.Value)}</td>
        </tr>`;
    });
...
<head>
  <style>
    @media print {
      ...
      .summary { margin-top:14px; font-size:9px; line-height:1.8; padding:0 8px; }
      .summary .line { display:flex; justify-content:space-between; max-width:500px; }
      .summary .note { font-size:8px; color:#555; margin-top:4px; }
      @page {
...
<body>
  <div class="title">All (GRN) Raised by EDL for Purchasing</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8.5px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:6px 8px; width:15%; text-align:right;">Total Value</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
      <tr>
        ...
        <td style="text-align:right; padding:6px 8px; border:1px solid #d1d5db; font-family:monospace;">${formatMoney(totalGrnValue)}</td>
      </tr>
    </tfoot>
  </table>

  <div class="summary">
    <div class="line"><span><strong>Total GRN value for the period</strong></span><span><strong>${formatMoney(totalGrnValue)}</strong></span></div>
    <div class="line"><span>** No. of GRNs raised for the period</span><span>${periodSummary?.grnCount ?? 0}</span></div>
    <div class="line"><span>** No. of issues raised for jobs and maintenance for the period</span><span>${periodSummary?.issueCount ?? 0}</span></div>
    <div class="line"><span>** Total issues value for the period</span><span>${formatMoney(periodSummary?.issueTotal ?? 0)}</span></div>
    <div class="note">** For all materials raised for the period</div>
  </div>
          ...
          <div className="relative bg-white w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] xl:w-[75vw] max-w-7xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-32 lg:mt-40 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
            {loadingReport && (
              <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-sm text-gray-600">Fetching GRN data from server</p>
              </div>
            )}
            ...
            {!loadingReport && reportData.length > 0 && (
              <div className="p-2 md:p-2 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible print:mt-10 print:ml-12">
                    ...
                    <table className="w-full text-xs border-collapse">
                      <thead className={`${maroonGrad} text-white`}>
                        <tr>
                          ...
                          <th className="px-4 py-2 border border-gray-300 text-right" style={{width: "15%"}}>Total Value</th>
                        </tr>
                      </thead>
                      <tbody>
                        {reportData.map((r, i) => (
                          <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            <td className="px-4 py-2 text-right font-mono border-r border-gray-300">{formatMoney(r.Value)}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="bg-[#d3d3d3] font-bold">
                          ...
                          <td className="px-4 py-2 text-right font-mono border border-gray-300">{formatMoney(totalGrnValue)}</td>
                        </tr>
                      </tfoot>
                    </table>
                ...
                {/* ── PERIOD SUMMARY ── */}
                <div className="ml-5 mr-12 mb-5 p-4 bg-gray-50 border border-gray-300 rounded-lg text-sm print:ml-12 print:mr-12">
                  <div className="flex justify-between items-center py-1">
                    <span className={`font-bold ${maroon}`}>Total GRN value for the period</span>
                    <span className="font-mono font-bold">{formatMoney(totalGrnValue)}</span>
                  </div>
                  <div className="border-t border-gray-200 mt-2 pt-2 space-y-1 text-gray-700">
                    <div className="flex justify-between items-center">
                      <span>** No. of GRNs raised for the period</span>
                      <span className="font-mono">{periodSummary?.grnCount ?? 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>** No. of issues raised for jobs and maintenance for the period</span>
                      <span className="font-mono">{periodSummary?.issueCount ?? 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>** Total issues value for the period</span>
                      <span className="font-mono">{formatMoney(periodSummary?.issueTotal ?? 0)}</span>
                    </div>
                  </div>
                  <p className="text-xs text-gray-500 mt-3">
                    ** For all materials raised for the period
                  </p>
                </div>
              </div>
            )}
...
export default GrnRaisedForPurchasingReport;