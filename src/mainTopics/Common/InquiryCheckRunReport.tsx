...
interface InquiryChequeRunItem {
...
/* ────── MAIN COMPONENT ────── */
const InquiryChequeRunReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryChequeRunItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquirychequerun/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryChequeRunItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY status,category,doc_pf,chq_run,run_dt ────── */
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Cheque payments / Temporary transaction inquiry report From ${fromDate} To ${toDate}`,
		...
		a.href = url;
		a.download = `InquiryChequeRun_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Cheque payments / Temporary transaction inquiry report From ${fromDate} To ${toDate}</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:12%;">Company</th>
        <th style="padding:5px 6px; width:12%;">Document No</th>
        <th style="padding:5px 6px; width:16%;">Type</th>
						...
						{reportLoading && (
							<div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
								...
								<p className="text-sm text-gray-600">
									Fetching cheque run data from server
								</p>
							</div>
						)}
...
export default InquiryChequeRunReport;