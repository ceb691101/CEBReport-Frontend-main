...
interface MaterialReqJobwiseItem {
	DocNo: string | null;
	DocPf: string | null;
	ReqDt: string | null;
	ResCd: string | null;
...
const normalizeProjectNo = (value: string): string =>
	value
		.trim()
		.replace(/\\/g, "/")
		.replace(/\s*\/\s*/g, "/")
...
/* ────── MAIN COMPONENT ────── */
const MaterialReqJobwiseReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<MaterialReqJobwiseItem[]>([]);
	...
	const validateInputs = (): string | null => {
		const normalizedProjectNo = normalizeProjectNo(projectNo);
		if (!normalizedProjectNo) {
			toast.error("Please enter a Project No.");
			return null;
		}
	...
	const fetchReport = async (dept: Department) => {
		const normalizedProjectNo = validateInputs();
		if (!normalizedProjectNo) return;
		...
		try {
			...
			const url = `/misapi/api/materialreqjobwise/report?${params.toString()}`;
			...
			const items: MaterialReqJobwiseItem[] = json.data || [];
			...
			if (items.length === 0) {
				if (normalizedProjectNo !== normalizeProjectNo(projectNo)) {
					toast.warn("The project number format was adjusted to match the backend format. Please try again.");
				} else {
	...
	/* ────── Single flat table, matches ORDER BY doc_pf,doc_no,res_cd ────── */
	const sortedData = [...reportData].sort(
		(a, b) =>
			(a.DocPf || "").localeCompare(b.DocPf || "") ||
			(a.DocNo || "").localeCompare(b.DocNo || "") ||
			(a.ResCd || "").localeCompare(b.ResCd || "")
	);

	const grandTotalReqCost = reportData.reduce((s, r) => s + (r.ReqCost || 0), 0);
	const grandTotalIssuedVal = reportData.reduce((s, r) => s + (r.IssuedVal || 0), 0);
	const searchedProjectNo = reportData.find((r) => r.ReqSource)?.ReqSource || projectNo;
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Material Requisition For Job No - ${searchedProjectNo}`,
			"",
		];
		...
		sortedData.forEach((it, i) => {
			rows.push(
				[
					...
					csvEscape(formatNumber(it.ReqCost)),
					csvEscape(it.ResCd),
					csvEscape(formatNumber(it.EstQty)),
					csvEscape(formatNumber(it.ComQty)),
					csvEscape(formatNumber(it.ReqUnits)),
					csvEscape(it.IssueDocPf),
					csvEscape(it.IssueDocNo),
					csvEscape(formatNumber(it.IssuedQty)),
					csvEscape(formatNumber(it.IssuedVal)),
		...
		rows.push(
			`Total,,,,${csvEscape(formatNumber(grandTotalReqCost))},,,,,,,,,${csvEscape(
				formatNumber(grandTotalIssuedVal)
			)},`
		);
		...
		a.href = url;
		a.download = `MaterialReqJobwise_${searchedProjectNo}.csv`;
		a.click();
...
<body>
  <div class="title">Material Requisition For Job No - ${searchedProjectNo}</div>
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:9%;">Req Doc No</th>
        <th style="padding:5px 6px; width:8%;">Req Doc Pf</th>
        <th style="padding:5px 6px; width:8%;">ReQ Date</th>
        <th style="padding:5px 6px; width:9%; text-align:right;">Req Amount</th>
        <th style="padding:5px 6px; width:8%;">Material Code</th>
        <th style="padding:5px 6px; width:8%; text-align:right;">Est Qty</th>
        <th style="padding:5px 6px; width:8%; text-align:right;">Commited Qty</th>
        <th style="padding:5px 6px; width:7%; text-align:right;">Req Qty</th>
        <th style="padding:5px 6px; width:8%;">Issue Doc Pf</th>
        <th style="padding:5px 6px; width:8%;">Issue Doc No</th>
        <th style="padding:5px 6px; width:8%; text-align:right;">Issued Qty</th>
        <th style="padding:5px 6px; width:9%; text-align:right;">Issued Value</th>
        <th style="padding:5px 6px; width:8%;">Status</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
      <tr>
        <td colspan="4" style="text-align:right; padding:5px 6px; border:1px solid #d1d5db;">Total</td>
        <td style="text-align:right; padding:5px 6px; border:1px solid #d1d5db; font-family:monospace;">${formatNumber(
			grandTotalReqCost
		)}</td>
        <td colspan="7" style="border:1px solid #d1d5db;"></td>
        <td style="text-align:right; padding:5px 6px; border:1px solid #d1d5db; font-family:monospace;">${formatNumber(
			grandTotalIssuedVal
		)}</td>
										...
										<table className="w-full text-xs border-collapse">
											<thead className={`${maroonGrad} text-white`}>
												<tr>
													...
													<th className="px-3 py-2 border border-gray-300 text-right" style={{width: "7%"}}>
														Req Qty
													</th>
													<th className="px-3 py-2 border border-gray-300" style={{width: "8%"}}>
														Issue Document Pf
											...
											<tbody>
												{sortedData.map((it, i) => (
													...
													>
														...
														<td className="px-3 py-2 text-right font-mono border-r border-gray-300">
															{formatNumber(it.IssuedQty)}
														</td>
														<td className="px-3 py-2 text-right font-mono border-r border-gray-300">
															{formatNumber(it.IssuedVal)}
														</td>
											...
											<tfoot>
												<tr className="bg-[#d3d3d3] font-bold">
													...
													<td className="px-3 py-2 text-right font-mono border border-gray-300">
														{formatNumber(grandTotalReqCost)}
													</td>
...
export default MaterialReqJobwiseReport;