// MaterialReqJobwiseNoMatReport.tsx
...
interface MaterialReqJobwiseNoMatItem {
...
/* ────── MAIN COMPONENT ────── */
const MaterialReqJobwiseNoMatReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<MaterialReqJobwiseNoMatItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/materialreqjobwisenomat/report?${params.toString()}`;
			...
			const items: MaterialReqJobwiseNoMatItem[] = json.data || [];
			if (items.length > MAX_RECORDS)
				throw new Error(
					`Too many records (${items.length}). Please refine your search.`
				);

			if (items.length === 0 && normalizedProjectNo !== normalizeProjectNo(projectNo)) {
	...
	/* ────── Single flat table, matches ORDER BY doc_pf,doc_no,req_dt ────── */
	...
	const downloadCSV = () => {
		...
		rows.push(`Total,,,${csvEscape(formatNumber(grandTotalReqCost))},,,,,,,`);
		...
		a.href = url;
		a.download = `MaterialReqJobwiseNoMat_${searchedProjectNo}.csv`;
		a.click();
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:9%;">Document Pf</th>
        <th style="padding:5px 6px; width:9%;">Req Date</th>
        <th style="padding:5px 6px; width:9%; text-align:right;">Req Amount</th>
        <th style="padding:5px 6px; width:10%;">Confirmed Date</th>
        <th style="padding:5px 6px; width:10%;">Qty Approved Date</th>
        <th style="padding:5px 6px; width:11%;">Issued/Returned Date</th>
        <th style="padding:5px 6px; width:10%;">Issued Document Pf</th>
        <th style="padding:5px 6px; width:10%;">Issued Document No</th>
    ...
    <tfoot>
      <tr>
        <td colspan="3" style="text-align:right; padding:5px 6px; border:1px solid #d1d5db;">Total</td>
		...
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>
					Material Requisition Jobwise (without Materials)
				</h2>
			</div>
			...
			{showReport && selectedDept && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/90 print:static print:inset-auto print:p-0 print:bg-white">
					<div className="relative bg-white w-[95vw] sm:w-[92vw] md:w-[88vw] lg:w-[84vw] xl:w-[80vw] max-w-[1400px] rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-16 md:mt-24 lg:mt-32 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
						{reportLoading && (
...
export default MaterialReqJobwiseNoMatReport;