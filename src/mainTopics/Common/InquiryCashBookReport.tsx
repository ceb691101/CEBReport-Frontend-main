...
/* ────── MAIN COMPONENT ────── */
const InquiryCashBookReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryCashBookItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquirycashbook/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryCashBookItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY status,category,doc_pf,doc_no,doc_dt ────── */
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Cash Book Transactions Inquiry Report - From ${fromDate} To ${toDate}`,
			`Cost Centre : ${costCenterDisplay} / ${costCenterName}`,
			"",
		];
		...
		a.href = url;
		a.download = `InquiryCashBook_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Cash Book Transactions Inquiry Report - From ${fromDate} To ${toDate}</div>
  <div class="info">
    <div><strong>Cost Centre :</strong> ${costCenterDisplay} / ${costCenterName}</div>
  </div>
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:18%;">Category</th>
        <th style="padding:5px 6px; width:13%;">Document No</th>
        <th style="padding:5px 6px; width:10%;">Type</th>
        <th style="padding:5px 6px; width:11%;">Transaction Date</th>
        <th style="padding:5px 6px; width:14%;">Checked By</th>
        <th style="padding:5px 6px; width:15%;">Approved By/Project No</th>
		...
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>
					Inquiry Cash Book (Pending & Cancellation)
				</h2>
			</div>
...
export default InquiryCashBookReport;