// InquiryMaterialRequisitionReport.tsx
...
interface InquiryMaterialRequisitionItem {
...
/* ────── MAIN COMPONENT ────── */
const InquiryMaterialRequisitionReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryMaterialRequisitionItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquirymaterialrequisition/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryMaterialRequisitionItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY status,doc_pf,doc_no,req_dt ────── */
	...
	const downloadCSV = () => {
		...
		a.href = url;
		a.download = `InquiryMaterialRequisition_${fromDate}_${toDate}.csv`;
		a.click();
...
export default InquiryMaterialRequisitionReport;