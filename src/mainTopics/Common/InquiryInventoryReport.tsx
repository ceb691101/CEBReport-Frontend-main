...
interface InquiryInventoryItem {
...
/* ────── MAIN COMPONENT ────── */
const InquiryInventoryReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryInventoryItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquiryinventory/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryInventoryItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY status,doc_pf,doc_no,trx_dt ────── */
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Issue Inventory Inquiry Report From ${fromDate} To ${toDate}`,
		...
		a.href = url;
		a.download = `InquiryInventory_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Issue Inventory Inquiry Report From ${fromDate} To ${toDate}</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:14%;">Type</th>
        ...
        <th style="padding:5px 6px; width:11%;">Status</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
						...
						{reportLoading && (
							<div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
								...
								<p className="text-sm text-gray-600">
									Fetching inventory data from server
								</p>
							</div>
						)}
...
export default InquiryInventoryReport;