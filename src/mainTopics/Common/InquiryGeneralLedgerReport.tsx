// InquiryGeneralLedgerReport.tsx
...
interface InquiryGeneralLedgerItem {
...
/* ────── MAIN COMPONENT ────── */
const InquiryGeneralLedgerReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<InquiryGeneralLedgerItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/inquirygeneralledger/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: InquiryGeneralLedgerItem[] = json.data || [];
	...
	/* ────── Single flat table, matches ORDER BY category,status,doc_pf,doc_no,doc_dt ────── */
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`General Ledger Transaction Inquiry Report From ${fromDate} To ${toDate}`,
		...
		a.href = url;
		a.download = `InquiryGeneralLedger_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">General Ledger Transaction Inquiry Report From ${fromDate} To ${toDate}</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:5px 6px; width:12%;">Document No</th>
        <th style="padding:5px 6px; width:9%;">Type</th>
        <th style="padding:5px 6px; width:11%;">Transaction Date</th>
        <th style="padding:5px 6px; width:12%;">Entered Date</th>
        <th style="padding:5px 6px; width:12%;">Checked By</th>
        <th style="padding:5px 6px; width:12%;">Approved By/Project No</th>
...
export default InquiryGeneralLedgerReport;